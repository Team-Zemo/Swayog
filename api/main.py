...
from logic.triangle_pose import TrianglePoseAngleChecker
...
# from logic.voice_feedback import VoiceFeedback

# JWT Configuration
JWT_SECRET = os.getenv("JWT_SECRET", "your-secret-key-must-be-at-least-256-bits-long-for-hs256-algorithm-please-change-in-production")
ALGORITHM = "HS512"
        ...
        try:
            while client_id in self.active_connections:
                ...
                if user_keypoints is None:
                    ...
                    # Speak the most important feedback
                    voice_message = ""
                    if feedback_lines:
                        # Speak the first (most critical) feedback line
                        voice_message = feedback_lines[0]
...
async def websocket_endpoint(websocket: WebSocket, client_id: str, token: Optional[str] = Query(None)):
    ...
    if token:
        ...
        # For now, we might allow unauthenticated connections for testing, 
        # but in production, you'd want to enforce this.
        # await websocket.close(code=status.WS_1008_POLICY_VIOLATION)
        # return
        pass
...
async def get_poses():
    ...
    return {"poses": list(pose_checkers.keys())}
